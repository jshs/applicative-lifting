...
Another problem is identifying terms of the form $\pure x$ and $f \ap x$.
We do not want to restrict the term structure of these ``constants'', since some
useful predefined concepts in the HOL libraries are abbreviations of compound terms.
...
pattern.
The matching algorithm does not work with arbitrary patterns, though. % TODO cite Nipkow 93
An obvious counterexample is the applicator pattern for the environment functor,
$\abs{x}{(v_1 x)(v_2 x)}$, which is not well-formed.
In such cases, one needs to introduce proper constants equal to the problematic
terms, and construct the goal equation explicitly with these constants.

Figure~\ref{tab:normalize-rules} has been extended by the corresponding HOL
equations.
These are the four applicative laws, which have to be supplied by the user.
The pure-rotate rule is easily proven with a custom rewriting tactic.
\todo
